...
public class DoctorMedicalRecordUI {
	...
	public DoctorMedicalRecordUI(Patient p) throws ParseException {
		...
		DoctorMedicalRecord.setSize(891, 701);
	...
	private void initialize(Patient p) throws ParseException {
		...
		splitPane.setBounds(24, 199, 837, 462);
				...
				for (int i = 1; i < specialty.length; i++) {
					...
					while (treatment.hasNext()) {
						...
						if (t.getDoctor().getSpeciality().equals(specialty[i])) {
							...
							Iterator<Examination> examination = t
		...
		btnNewTreatment.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					Treatment treatment = mrctrl.getNewTreatment((Doctor) Application.user,p.getMedicalRecord());
	...
	public void createtree(String id) {
		 ...
		 * =p.getMedicalRecord().getTreatments().iterator();
		 * DefaultMutableTreeNode node_2 = new DefaultMutableTreeNode("Visits");
		 * DefaultMutableTreeNode node_3 = new
		 * DefaultMutableTreeNode("Examinations");
		 * 
		 * while ( treatment.hasNext() ) {
		 * 
		 * Treatment t =treatment.next();
		 ...
		 * if (t.getDoctor().getSpeciality().equals(specialty[i])) {
		 * 
		 * node_1 = new DefaultMutableTreeNode(t); //DefaultMutableTreeNode
		 * node_2 = new DefaultMutableTreeNode("Visits"); Iterator <Visit> visit
		 ...
		 * examination =t.getExamination().iterator(); while
		 * (examination.hasNext()) { Examination ex =examination.next();
		 * node_3.add(new DefaultMutableTreeNode(ex)); }
		 * 
