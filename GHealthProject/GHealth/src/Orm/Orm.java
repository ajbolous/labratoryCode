...
public class Orm {
	...
	public String createSql(Class<?> c) {
		String name = c.getName().replace("models.", "").toLowerCase() + "s"; // get
																				// table
																				// name
		String columns = "";
		String primaries = "";
		...
		for (Field field : fields) {
			...
			if (field.isAnnotationPresent(fkField.class)) {
				...
				foreigns += "FOREIGN KEY(" + field.getName() + ") REFERENCES "
						+ f[0].target() + ",";
			}
		}

		if (c.isAnnotationPresent(extensionTable.class)) {
			extensionTable[] f = (extensionTable[]) c
					.getAnnotationsByType(extensionTable.class);
			columns = f[0].field() + " " + dataType(String.class) + ","
					+ columns;
			foreigns = "FOREIGN KEY(" + f[0].field() + ") REFERENCES "
					+ f[0].table() + "(" + f[0].field() + ")," + foreigns;
	...
	public String insertSql(Class<?> c) {
		...
		String s = String.format("INSERT INTO %s(%s) VALUES(%s);", name,
	...
	public String querySql(Class<?> c) {
		...
		while (c.getName() != Entity.class.getName()) {
			String tblName = c.getName().replace("models.", "").toLowerCase()
					+ "s";

			for (Field field : c.getDeclaredFields())
				if (field.isAnnotationPresent(dataField.class))
					fields += tblName + "." + field.getName() + " as "
							+ field.getName() + ",";

			if (c.isAnnotationPresent(extensionTable.class)) {
				extensionTable[] f = (extensionTable[]) c
						.getAnnotationsByType(extensionTable.class);
				joins = joins + tblName + "." + f[0].field() + "="
						+ f[0].table() + "." + f[0].field() + " AND ";
			}
			tables += tblName + ",";
			c = c.getSuperclass();
		}
		...
		String sql = String.format("SELECT %s FROM %s %s",
				fields.substring(0, fields.length() - 1), tables, joins);
		return sql;
	}
	...
	public ArrayList<Entity> getObject(Class<?> c, String where)
			throws Exception {
	...
	public Method getGetter(Class<?> c, String attr) throws Exception {
		String mName = "get" + attr.substring(0, 1).toUpperCase()
				+ attr.substring(1, attr.length());
		return c.getMethod(mName);
	}

	public Method getSetter(Class<?> c, String attr, Class<?> attrType)
			throws Exception {
		String mName = "set" + attr.substring(0, 1).toUpperCase()
				+ attr.substring(1, attr.length());
		return c.getMethod(mName, attrType);
	}

	public PreparedStatement fromObject(PreparedStatement stmt, Object obj,
	...
	public Entity fromResult(ResultSet rs, Class<?> c) throws Exception {
		...
		while (c.getName() != Entity.class.getName()) {
			for (Field field : c.getDeclaredFields())
				if (field.isAnnotationPresent(dataField.class)) {
					Method setter = getSetter(c, field.getName(),
							field.getType());
